...
public class SbeaverOnKeyPressedProcedure {
				...
				for (Entity entityiterator : _entfound) {
					if (entityiterator instanceof RadioBeaverEntity) {
						if ((entityiterator instanceof LivingEntity _entGetArmor ? _entGetArmor.getItemBySlot(EquipmentSlot.HEAD) : ItemStack.EMPTY).getItem() == JigsawModItems.RADIOHAT_HELMET.get()
							...
							entityiterator
									.setDeltaMovement(new Vec3((speed2 * Math.cos((entityiterator.getYRot() - 90) * (Math.PI / 180))), (entityiterator.getDeltaMovement().y()), (speed2 * Math.sin((entityiterator.getYRot() - 90) * (Math.PI / 180)))));
						}
					}
				}
			}
