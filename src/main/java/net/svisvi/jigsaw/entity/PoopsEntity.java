...
import net.svisvi.jigsaw.procedures.PoopsWhileProjectileFlyingTickProcedure;
import net.svisvi.jigsaw.procedures.PoopsProjectileHitsLivingEntityProcedure;
import net.svisvi.jigsaw.procedures.PoopsProjectileHitsBlockProcedure;
...
public class PoopsEntity extends AbstractArrow implements ItemSupplier {
	public PoopsEntity(PlayMessages.SpawnEntity packet, Level world) {
		super(JigsawModEntities.POOPS.get(), world);
	}
	...
	public ItemStack getItem() {
		return new ItemStack(JigsawModItems.POOPS_ITEM.get());
	}
	...
	public void tick() {
		super.tick();
		PoopsWhileProjectileFlyingTickProcedure.execute(this.level, this.getX(), this.getY(), this.getZ(), this.getOwner(), this);
		if (this.inGround)
			this.discard();
	}

	public static PoopsEntity shoot(Level world, LivingEntity entity, Random random, float power, double damage, int knockback) {
		PoopsEntity entityarrow = new PoopsEntity(JigsawModEntities.POOPS.get(), entity, world);
	...
	public static PoopsEntity shoot(LivingEntity entity, LivingEntity target) {
		PoopsEntity entityarrow = new PoopsEntity(JigsawModEntities.POOPS.get(), entity, entity.level);
